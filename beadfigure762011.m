function [ y sd ] = beadfigure(conc,exposures,groupselected )
...
x=conc;
whichgroup=sprintf('C:\\work\\beadanalysis\\group %1.0f.txt',(groupselected-1));
S = importdata(char(whichgroup));
LP = [1]; 
SD=[2];
cAOI= [4];
...
m = size(S.data);
m1 = m(1);
mat = zeros(m1,length(qcvar));
% sdmat=zeros(m1,length(qcvar));
skipsize=numel(numberofexposures);
for i = 1:length(qcvar)
mat(:,i) = S.data(:,qcvar(i));
% sdmat(:,i) = S.data(:,qcvar(i));
end

numberofconc=length(conc);
y=zeros(skipsize,numberofconc);
startexposure=numberofexposures(1);
for p=1:skipsize  
    y(p,:)=mat([p:skipsize:m1],1);
end
...
for s=1:skipsize
    ...
    yval=y(s,:);
    axes(a{s}), dose_response(x,yval), title(sptit), xlabel('Concentration'), ylabel('Intensity')
end
